...
| Radio Browser discovery through `curl` | `network.fetch` generation 1 / `6ab7…b6a9`, operation `fetch`, scoped to exact HTTPS origins and `GET`; the intended trusted Radio provider fixes the origin and path, bounds the normalized result to 64 records, accepts only HTTPS stream URLs, and returns opaque playback handles | QML request and response handling are ported; the final trusted provider is not yet installed or exercised with this branch |
