...
| Radio Browser discovery through `curl` | `network.fetch` scoped to exact HTTPS origins and `GET`; a separately installed Radio Browser adapter profile fixes paths, redirect/DNS policy, response size, record count, cache rules, and operations | Generic authority is requested; Radio Browser is adapter identity, never permission vocabulary |
| Stream playback through plugin scripts, proxy, `mpv`, and MPRIS | `media.play-stream` constrained to opaque handles issued by the granted fetch adapter, with enumerated media controls | Required adapter is not implemented; passing community stream URLs back from QML would be the wrong boundary |
...
| Screensaver observation through raw Hyprland events | `system.observe` scoped to the sanitized `screensaver.state` dataset | Optional adapter is not implemented; omission only removes auto-dismiss |
