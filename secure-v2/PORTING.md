...
| `PanelWindow`, layer-shell namespace, input mask, and keyboard focus | Host-owned `desktop-overlay` surface with the original UI's 1180×760 allocation, bounded frame rate, focus-after-gesture, and dynamic bounded input regions | Declared for the worker's multi-surface path; the allocation keeps the shell bar outside Radio's trusted input region |
