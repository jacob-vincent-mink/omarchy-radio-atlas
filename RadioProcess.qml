...
QtObject {
  ...
  property RadioOutput stdout: null
  property RadioOutput stderr: null
  ...
  function finish(exitCode, output, error) {
    if (!running) return
    if (stdout) stdout.publish(String(output || ""))
    if (stderr) stderr.publish(String(error || ""))
    running = false
    exited(exitCode)
  }

  function watch(call, success, failure) {
    if (!call) { failure("Broker request was rejected"); return }
    var generation = requestGeneration
    pendingCalls = pendingCalls.concat([call])
    var done = function() {
      if (!call.finished) return
      try { call.finishedChanged.disconnect(done) } catch (_) {}
      root.pendingCalls = root.pendingCalls.filter(function(item) { return item !== call })
      if (!root.running || generation !== root.requestGeneration) return
      if (!call.ok) { failure(String(call.error || "Request failed")); return }
      try { success(String(call.utf8Text || "")) }
  ...
  function observe(call, transform) {
    watch(call, function(text) { finish(0, transform(text), "") },
  ...
  function currentCallback(callback) {
    ...
    return function(value) {
      if (root.running && generation === root.requestGeneration) callback(value)
    }
  }
  ...
  property Qml.Connections readiness: Qml.Connections {
    target: runtime
    function onBrokerReadyChanged() { root.start() }
  }
  ...
  function requestStations(path, onSuccess, onFailure) {
    var request = runtime.invoke("network.fetch", "fetch", {
      method: "GET", origin: "https://all.api.radio-browser.info", path: path,
        headers: {accept: "application/json"}, responseType: "json",
        mediaJsonPointers: ["/*/url_resolved"]
    })
    watch(request, function(text) {
      try {
  ...
  function mediaControl(control, value) {
    ...
    observe(runtime.invoke("media.play-stream", "control", payload), function(raw) {
      if (control === "volume") {
  ...
  function playStation(station) {
    ...
    observe(runtime.invoke("media.play-stream", "play", {
      handle: station.playbackHandle, volume: RadioSessionState.volume
    }), function(raw) {
      var state = JSON.parse(raw || "{}")
  ...
  function runPlayer(args) {
    ...
    if (action === "outputs" || action === "output") {
      finish(1, "", "Audio output selection is unavailable in the v2 provider")
      return
    }
    if (action === "play") {
  ...
  function persistState() {
    ...
    RadioStorage.save(RadioSessionState.favorites, RadioSessionState.recent, RadioSessionState.volume,
      currentCallback(function(ok) { if (ok) finish(0, JSON.stringify(stateObject()), ""); else finish(1, "", "Could not save state") }))
  }
  ...
  function runState(args) {
    ...
    if (action === "get") {
      RadioStorage.load(currentCallback(function(saved) {
  ...
  function start() {
    if (!initialized || !running || dispatched || !runtime.brokerReady) return
    dispatched = true
    if (command.length < 2 || command.length > 4) {
      finish(1, "", "Invalid radio operation")
      return
    }
